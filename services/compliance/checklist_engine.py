...
class ComplianceChecklistEngine:
    ...
    def _load_all_checklists(self):
        ...
        print(f"Loaded {len(self.checklists)} compliance checklists from {len(list(self.plugin_directory.iterdir()))} industries")
    ...
    def _evaluate_condition(self, condition: str, value: Any) -> bool:
        ...
        # Replace 'value' in condition string with actual value
        try:
            condition = condition.replace("value", str(value))
            return eval(condition)  # Warning: eval is dangerous, use carefully
        except:
            return False
